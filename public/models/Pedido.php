...
class Pedido
{
    ...
    public function crearUno()
    {
        ...
        $consulta = $objAccesoDatos->prepararConsulta("INSERT INTO pedidos (codigoPedido, producto, cantidad, mesa, mozo, fecha, estado, sector,tiempoPreparacion,tiempoEntrega) VALUES (:codigoPedido, :producto, :cantidad, :mesa, :mozo, :fecha, :estado, :sector, :tiempoPreparacion, :tiempoEntrega)");
        // echo($this->producto);
        ...
        $consulta->bindValue(':tiempoEntrega', $this->tiempoEntrega, PDO::PARAM_INT);
    ...
    public static function obtenerTodos($estado, $sector)
    {
        ...
        // Consulta base
        $bandera=0;
        $consultaString = "SELECT p.id, p.codigoPedido, p.cantidad, p.mesa, u.nombre as mozo, p.fecha, p.estado, p.sector, pr.descripcion as producto,p.tiempoPreparacion, p.tiempoEntrega FROM pedidos p
    ...
    {
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("SELECT id, codigoPedido, producto, cantidad, mesa, mozo, fecha, estado,sector,tiempoPreparacion,tiempoEntrega FROM pedidos WHERE id = :id");
    ...
    {
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("SELECT id, codigoPedido, producto, cantidad, mesa, mozo, fecha, estado,sector,tiempoPreparacion,tiempoEntrega FROM pedidos WHERE codigoPedido = :codigoPedido");
    ...
    {
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("UPDATE pedidos SET producto = :producto, cantidad = :cantidad, mesa = :mesa, mozo = :mozo, fecha = :fecha, estado = :estado, sector = :sector, codigoPedido = :codigoPedido , tiempoPreparacion = :tiempoPreparacion, tiempoEntrega = :tiempoEntrega WHERE id = :id");
    ...
    {
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("UPDATE pedidos SET estado = 'cancelado' WHERE id = :id");
